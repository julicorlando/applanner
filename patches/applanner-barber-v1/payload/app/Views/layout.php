<?php
use App\Core\{Auth,CSRF,Authorization,SubscriptionAccess,Database};
use App\Services\{ModuleService,LegalDocumentService,ReferralAttributionService,ArenaTenantService};
...
$isBarberTenant=!$isSportsTenant&&in_array($normalizedTenantCategory,['barbearia','barber','barbershop'],true);
...
$headerEyebrow=$restricted?'Acesso para regularização':($isPlatform?'Administração':($isSportsTenant?'Gestão da Arena':($isBarberTenant?'Gestão da Barbearia':((($user['role']??'')==='professional')?'Área do profissional':'Operação'))));
$brandSubtitle=$isPlatform?'Administração da plataforma':($isSportsTenant?'ApPlanner Arena':($isBarberTenant?'ApPlanner Barber':((($user['role']??'')==='professional')?'Portal profissional':'Gestão inteligente')));
...
if($isPlatform){
    $nav=[];
    if(($user['role']??'')==='master'){
        $nav=[['Painel','/master'],['Operação comercial','/master/analytics'],['Clientes da plataforma','/master/tenants'],['Usuários','/master/users'],['Planos','/master/plans'],['Catálogo de módulos','/master/modulos'],['Solicitações de módulos','/master/solicitacoes-modulos'],['Financeiro da plataforma','/master/financeiro'],['Pagamentos','/master/payments'],['Contas bloqueadas','/master/blocked'],['Suporte','/master/support'],['Auditoria de acesso','/master/login-audit'],['Automações e disparos','/master/automation-logs'],['Integrações','/master/integrations'],['Integrações Bancárias','/master/banking-integrations'],['Chatbot WhatsApp','/master/whatsapp-chatbot'],['Mercado Pago','/master/mercadopago'],['Documentos legais','/master/documentos-legais'],['Backups','/master/backups'],['Central de erros','/master/errors'],['Saúde do sistema','/master/system-health']];
        array_splice($nav,1,0,[["Homologação e operação","/master/homologacao"],["Central de incidentes","/master/incidentes"],["Central comercial","/master/comercial"],["Oportunidades comerciais","/comercial/oportunidades"]]);
        array_splice($nav,array_search('Backups',array_column($nav,0),true)+1,0,[["Implantação de bases","/master/imports"]]);
        array_splice($nav,array_search('Operação comercial',array_column($nav,0),true)+1,0,[["Blog","/master/blog"]]);
        array_splice($nav,array_search('Blog',array_column($nav,0),true)+1,0,[["E-mail marketing","/master/email-marketing"]]);
...
}elseif(($user['role']??'')==='professional'&&!$isSportsTenant){
...
<?php if(!$restricted&&!$isPlatform):?><nav class="mobile-bottom-nav" aria-label="Atalhos"><a href="<?=$isSportsTenant?'/sports':'/dashboard'?>">Início</a><a href="<?=$isSportsTenant?'/sports/reservations':'/appointments'?>"><?=$isSportsTenant?'Reservas':'Agenda'?></a><?php if(($user['role']??'')!=='professional'):?><a class="new" href="<?=$isSportsTenant?'/sports/reservations':'/appointments/create'?>">+</a><a href="/customers">Clientes</a><?php elseif(!empty($modules['products'])):?><a class="new" href="/sales">$</a><a href="/professional/availability">Horários</a><?php endif;?><button data-sidebar-toggle>Menu</button></nav><?php endif;?>
...
<div class="command-dialog" data-command-dialog hidden><div class="command-panel" role="dialog" aria-modal="true" aria-label="Busca rápida"><div class="command-input"><span>⌕</span><input data-command-input placeholder="<?=$isSportsTenant?'Buscar clientes e reservas':'Buscar clientes e agenda'?>" aria-label="Buscar"><button data-command-close aria-label="Fechar">Esc</button></div><div class="command-results"><a href="/customers">Clientes</a><?php if($isSportsTenant):?><a href="/sports/reservations">Reservas de quadras</a><a href="/sports/courts">Quadras e horários</a><?php elseif($isBarberTenant):?><a href="/appointments">Agenda</a><a href="/barber/commands">Comandas</a><a href="/barber/queue">Fila / Encaixes</a><?php else:?><a href="/appointments">Agenda</a><a href="/services">Serviços</a><?php endif;?></div></div></div><div class="toast-region" aria-live="polite" aria-atomic="true"></div><script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script><script src="/public/assets/js/app.js?v=20260808-theme-auto"></script><script src="/public/assets/js/sidebar-collapse.js?v=20260813"></script></body></html>
