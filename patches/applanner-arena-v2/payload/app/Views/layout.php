...
$headerEyebrow=$restricted?'Acesso para regularização':($isPlatform?'Administração':($isSportsTenant?'Gestão da Arena':((($user['role']??'')==='professional')?'Área do profissional':'Operação')));
$brandSubtitle=$isPlatform?'Administração da plataforma':($isSportsTenant?'ApPlanner Arena':((($user['role']??'')==='professional')?'Portal profissional':'Gestão inteligente'));
