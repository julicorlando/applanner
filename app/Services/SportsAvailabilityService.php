...
final class SportsAvailabilityService
{
    ...
    public function slots(int $tenant, int $court, int $modality, string $date, int $duration, ?int $excludeReservationId = null, ?int $excludeWaitlistId = null): array
    {
        ...
        foreach ($hours as $hoursRow) {
            ...
            while ($cursor->modify('+' . $duration . ' minutes') <= $limit) {
                ...
                if (
                    ...
                    && $this->free($tenant, $court, $cursor, $end, (int)$courtRow['interval_minutes'], $excludeReservationId, $excludeWaitlistId)
                ) {
                    ...
                    if ($quote !== null) {
                        $out[] = [
                            ...
                            'base_total' => $quote['base_total'] ?? $quote['total'],
                            'base_formatted' => 'R$ ' . number_format((float)($quote['base_total'] ?? $quote['total']), 2, ',', '.'),
                            'pricing_multiplier' => $quote['multiplier'] ?? 1,
                            'dynamic_pricing' => !empty($quote['enabled']) && abs((float)($quote['total'] ?? 0) - (float)($quote['base_total'] ?? $quote['total'])) > 0.009,
                            'pricing_adjustments' => $quote['adjustments'] ?? [],
                        ];
                    }
                }
    ...
    public function quote(int $tenant, int $court, int $modality, \DateTimeImmutable $start, int $duration): ?array
    {
        ...
        $baseTotal = round($hour * $duration / 60, 2);
        return (new ArenaDynamicPricingService())->apply($tenant, $court, $modality, $start, $duration, $hour, $baseTotal);
    }
    ...
    public function free(
        ...
        // A janela exclusiva da lista de espera segura o slot para o cliente ofertado.
        $holdSql = "SELECT 1 FROM sports_waitlist
                    WHERE tenant_id=:t AND court_id=:c AND status='offered' AND offer_expires_at>NOW()
                      AND offered_starts_at IS NOT NULL AND offered_starts_at<:ends
                      AND DATE_ADD(offered_starts_at,INTERVAL duration_minutes MINUTE)>:starts";
        $holdParams = ['t'=>$tenant,'c'=>$court,'ends'=>$to,'starts'=>$from];
        if ($excludeWaitlistId !== null && $excludeWaitlistId > 0) {
            $holdSql .= ' AND id<>:exclude_waitlist';
            $holdParams['exclude_waitlist'] = $excludeWaitlistId;
        }
        $holdSql .= ' LIMIT 1';
        try {
            $hold = $pdo->prepare($holdSql); $hold->execute($holdParams);
            if ($hold->fetchColumn()) return false;
        } catch (\Throwable) {
            // Compatibilidade com instalações que ainda não aplicaram a migration 038.
        }
