...
$stale=$count("SELECT COUNT(*) FROM jobs WHERE status='processing' AND locked_at<DATE_SUB(NOW(),INTERVAL 15 MINUTE)");
$add('Fila','Sem jobs falhos',$failed===0,$failed.' job(s) falho(s)');
...
$providerRefs=$count("SELECT COUNT(*) FROM subscriptions WHERE provider_subscription_id IS NOT NULL AND TRIM(provider_subscription_id)<>''");
$add('Billing','Assinatura com referência de provider',$providerRefs>0,$providerRefs.' assinatura(s)',$levelForCommercial());
$paidPayments=$count("SELECT COUNT(*) FROM payments WHERE status='paid'");
...
$moduleApplied=$count("SELECT COUNT(*) FROM subscription_module_adjustments WHERE status='applied'");
...
$tenantConnections=$count("SELECT COUNT(*) FROM tenant_payment_connections WHERE provider='mercadopago' AND status='connected'");
...
$tenantPaid=$count("SELECT COUNT(*) FROM tenant_payment_transactions WHERE status='paid'");
...
$tenantWebhook=$count("SELECT COUNT(*) FROM tenant_payment_webhook_events WHERE signature_valid=1 AND status='processed'");
...
if($external && $tenantConnections>0){
    try{
        $rows=$pdo->query("SELECT id,tenant_id,environment,credentials_encrypted FROM tenant_payment_connections WHERE provider='mercadopago' AND status='connected' ORDER BY (environment='production') DESC,id LIMIT 5")->fetchAll()?:[];
        $tested=0;$errors=[];
        foreach($rows as $row){
            try{
