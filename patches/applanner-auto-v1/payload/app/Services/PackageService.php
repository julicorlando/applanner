...
final class PackageService
{
    ...
    public static function hasVehicleBalance(int $tenantId,int $customerId,int $serviceId,int $vehicleId):bool
    {
        if(!ModuleService::has('packages',$tenantId))return false;
        $q=Database::connection()->prepare("SELECT 1 FROM customer_packages cp JOIN service_package_items spi ON spi.package_id=cp.package_id AND spi.service_id=:s LEFT JOIN auto_vehicle_package_links av ON av.customer_package_id=cp.id AND av.tenant_id=cp.tenant_id WHERE cp.tenant_id=:t AND cp.customer_id=:c AND cp.status='active' AND (cp.expires_at IS NULL OR cp.expires_at>=NOW()) AND (av.vehicle_id IS NULL OR av.vehicle_id=:v) AND (SELECT COALESCE(SUM(u.quantity),0) FROM customer_package_usage u WHERE u.customer_package_id=cp.id AND u.service_id=:s2)<spi.quantity LIMIT 1");
        $q->execute(['s'=>$serviceId,'s2'=>$serviceId,'t'=>$tenantId,'c'=>$customerId,'v'=>$vehicleId]);return (bool)$q->fetchColumn();
    }
    ...
    public static function consumeForAppointment(int $tenantId,int $appointmentId,int $customerId,int $serviceId):bool
    {
        ...
        try{
            ...
            $vq=$pdo->prepare('SELECT vehicle_id FROM appointments WHERE id=:a AND tenant_id=:t');$vq->execute(['a'=>$appointmentId,'t'=>$tenantId]);$vehicle=$vq->fetchColumn();$vehicle=$vehicle!==false&&$vehicle!==null?(int)$vehicle:null;
            $q=$pdo->prepare("SELECT cp.id,cp.package_id,spi.quantity included,(SELECT COALESCE(SUM(u.quantity),0) FROM customer_package_usage u WHERE u.customer_package_id=cp.id AND u.service_id=:service_used) used FROM customer_packages cp JOIN service_package_items spi ON spi.package_id=cp.package_id AND spi.service_id=:service_item LEFT JOIN auto_vehicle_package_links av ON av.customer_package_id=cp.id AND av.tenant_id=cp.tenant_id WHERE cp.tenant_id=:t AND cp.customer_id=:c AND cp.status='active' AND (cp.expires_at IS NULL OR cp.expires_at>=NOW()) AND (av.vehicle_id IS NULL OR (:vehicle IS NOT NULL AND av.vehicle_id=:vehicle2)) HAVING used<included ORDER BY COALESCE(cp.expires_at,'9999-12-31 23:59:59'),cp.id LIMIT 1 FOR UPDATE");
            $q->execute(['service_used'=>$serviceId,'service_item'=>$serviceId,'t'=>$tenantId,'c'=>$customerId,'vehicle'=>$vehicle,'vehicle2'=>$vehicle]);$cp=$q->fetch();if(!$cp){if($ownTx)$pdo->commit();return false;}
