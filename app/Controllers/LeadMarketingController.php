...
final class LeadMarketingController
{
    ...
    public function index():void
    {
        ...
        $campaigns=$pdo->query("SELECT c.*,u.name author_name,ru.name referrer_name,cr.referrer_user_id,
            (SELECT COUNT(*) FROM marketing_referral_visits rv WHERE rv.campaign_id=c.id) visit_count,
            (SELECT COUNT(DISTINCT rv.lead_id) FROM marketing_referral_visits rv WHERE rv.campaign_id=c.id AND rv.lead_id IS NOT NULL) unique_clicks,
            (SELECT MAX(rv.clicked_at) FROM marketing_referral_visits rv WHERE rv.campaign_id=c.id) last_click_at,
            (SELECT COUNT(DISTINCT rv.lead_id) FROM marketing_referral_visits rv WHERE rv.campaign_id=c.id AND rv.lead_id IS NOT NULL AND rv.converted_tenant_id IS NOT NULL) converted_clicks
            FROM marketing_campaigns c JOIN users u ON u.id=c.created_by
            LEFT JOIN marketing_campaign_referrers cr ON cr.campaign_id=c.id
            LEFT JOIN users ru ON ru.id=cr.referrer_user_id
            WHERE c.deleted_at IS NULL ORDER BY c.id DESC LIMIT 80")->fetchAll();
        ...
        $clickCampaign=null;$clickDetails=[];$clickId=filter_var($_GET['clicks_campaign']??null,FILTER_VALIDATE_INT);
        if($clickId){
            $q=$pdo->prepare("SELECT id,subject,sent_count,total_count FROM marketing_campaigns WHERE id=:id AND deleted_at IS NULL LIMIT 1");
            $q->execute(['id'=>$clickId]);$clickCampaign=$q->fetch()?:null;
            if($clickCampaign){
                $q=$pdo->prepare("SELECT rv.lead_id,COALESCE(ml.name,'Contato removido') lead_name,COALESCE(ml.email,'—') lead_email,
                    COUNT(rv.id) clicks,MIN(rv.clicked_at) first_click_at,MAX(rv.clicked_at) last_click_at,
                    MAX(rv.converted_tenant_id IS NOT NULL) converted
                    FROM marketing_referral_visits rv LEFT JOIN marketing_leads ml ON ml.id=rv.lead_id
                    WHERE rv.campaign_id=:id
                    GROUP BY rv.lead_id,ml.name,ml.email ORDER BY last_click_at DESC LIMIT 300");
                $q->execute(['id'=>$clickId]);$clickDetails=$q->fetchAll();
            }
        }
        View::render('master/lead-marketing',['title'=>'E-mail marketing','smtp'=>$smtp,'smtpConfigured'=>!empty($smtp['host'])&&!empty($smtp['from_email']),'stats'=>$stats,'leads'=>$leads,'campaigns'=>$campaigns,'staff'=>$staff,'editCampaign'=>$editCampaign,'clickCampaign'=>$clickCampaign,'clickDetails'=>$clickDetails]);
    }
    ...
    public function send():void
    {
        ...
        try{
            ...
            foreach($leads as $i=>$lead){
                ...
                $payload=Encryption::encrypt(['campaign_id'=>$campaignId,'delivery_id'=>$deliveryId,'lead_id'=>(int)$lead['id'],'to'=>$lead['email'],'subject'=>$subject,'message'=>$message,'image_url'=>$stored['url']??null,'image_alt'=>$stored?$imageAlt:null,'card_link_url'=>$stored?($cardLink!==''?$referralUrl.'&dest=card':$referralUrl):null,'referral_url'=>$referralUrl,'referrer_name'=>$referrer['name'],'unsubscribe_url'=>$base.'/email/descadastrar/'.$lead['unsubscribe_token']]);
    ...
    public function redispatchCampaign(string $id):void
    {
        $this->master();CSRF::enforce();$sourceId=(int)$id;$pdo=Database::connection();$source=$this->campaign($pdo,$sourceId);
        $referrerId=(int)($source['referrer_user_id']??0);$referrer=$this->referrer($pdo,$referrerId);

        // Impede duplicidade acidental enquanto a rodada original ainda possui e-mails aguardando envio.
        $queued=$this->countQueuedCampaignJobs($pdo,$sourceId);
        if($queued>0)HttpException::abort(409,'A campanha ainda possui '.$queued.' e-mail(s) na fila. Aguarde concluir ou inative antes de redisparar.');

        $leads=$pdo->query("SELECT id,name,email,unsubscribe_token FROM marketing_leads WHERE status='active' ORDER BY id LIMIT 5000")->fetchAll();
        if(!$leads)HttpException::abort(422,'Não há contatos ativos para o redisparo.');

        $rootSourceId=(int)($source['source_campaign_id']??0)>0?(int)$source['source_campaign_id']:$sourceId;
        $seq=$pdo->prepare("SELECT COALESCE(MAX(resend_number),0)+1 FROM marketing_campaigns WHERE (source_campaign_id=:root OR id=:root2) AND deleted_at IS NULL");
        $seq->execute(['root'=>$rootSourceId,'root2'=>$rootSourceId]);$resendNumber=max(1,(int)$seq->fetchColumn());

        $app=require __DIR__.'/../../config/app.php';$base=rtrim((string)($app['url']??'https://applanner.com.br'),'/');
        $pdo->beginTransaction();
        try{
            $pdo->prepare("INSERT INTO marketing_campaigns(subject,body,image_path,image_url,image_alt,card_link_url,active,status,total_count,sent_count,failed_count,created_by,created_at,queued_at,updated_at,source_campaign_id,resend_number,resent_at) VALUES(:s,:b,:path,:url,:alt,:link,1,'sending',:total,0,0,:u,NOW(),NOW(),NOW(),:source,:seq,NOW())")
                ->execute([
                    's'=>$source['subject'],'b'=>$source['body'],'path'=>$source['image_path']??null,'url'=>$source['image_url']??null,
                    'alt'=>$source['image_alt']??null,'link'=>$source['card_link_url']??null,'total'=>count($leads),'u'=>Auth::user()['id'],
                    'source'=>$rootSourceId,'seq'=>$resendNumber
                ]);
            $campaignId=(int)$pdo->lastInsertId();
            $pdo->prepare("INSERT INTO marketing_campaign_referrers(campaign_id,referrer_user_id,created_at)VALUES(:c,:u,NOW())")->execute(['c'=>$campaignId,'u'=>$referrerId]);
            ...
            foreach($leads as $i=>$lead){
                $delivery->execute(['c'=>$campaignId,'l'=>$lead['id']]);$deliveryId=(int)$pdo->lastInsertId();
                $referralUrl=$base.'/indicacao/'.$referrer['referral_code'].'?c='.$campaignId.'&l='.(int)$lead['id'];
                $message=str_replace(['{nome}','{email}','{link_indicacao}'],[$lead['name'],$lead['email'],$referralUrl],(string)$source['body']);
                $payload=Encryption::encrypt([
                    'campaign_id'=>$campaignId,'delivery_id'=>$deliveryId,'lead_id'=>(int)$lead['id'],'to'=>$lead['email'],
                    'subject'=>(string)$source['subject'],'message'=>$message,'image_url'=>$source['image_url']??null,
                    'image_alt'=>$source['image_alt']??null,'card_link_url'=>!empty($source['image_url'])?(!empty($source['card_link_url'])?$referralUrl.'&dest=card':$referralUrl):null,
                    'referral_url'=>$referralUrl,'referrer_name'=>$referrer['name'],
                    'unsubscribe_url'=>$base.'/email/descadastrar/'.$lead['unsubscribe_token']
                ]);
                $available=date('Y-m-d H:i:s',time()+intdiv($i,20)*60);$job->execute(['p'=>$payload,'available'=>$available]);
            }
            $pdo->commit();
        ...
        Audit::log('MARKETING_CAMPAIGN_REDISPATCHED','marketing_campaigns',$campaignId,null,[
            'source_campaign_id'=>$sourceId,'root_source_campaign_id'=>$rootSourceId,'resend_number'=>$resendNumber,
            'recipients'=>count($leads),'referrer_user_id'=>$referrerId
        ]);
        header('Location: /master/email-marketing?campaign_action=redispatched&campaign_id='.$campaignId);exit;
    }
    ...
    private function rewriteQueuedJobs(\PDO $pdo,array $campaign,array $referrer):int
    {
        ...
        foreach($jobs as $job){try{$p=Encryption::decrypt($job['payload_encrypted']);}catch(\Throwable){continue;}if((int)($p['campaign_id']??0)!==(int)$campaign['id'])continue;$leadStmt->execute(['id'=>(int)($p['lead_id']??0)]);$lead=$leadStmt->fetch();if(!$lead)continue;$referralUrl=$base.'/indicacao/'.$referrer['referral_code'].'?c='.(int)$campaign['id'].'&l='.(int)$lead['id'];$message=str_replace(['{nome}','{email}','{link_indicacao}'],[$lead['name'],$lead['email'],$referralUrl],(string)$campaign['body']);$p=array_merge($p,['to'=>$lead['email'],'subject'=>(string)$campaign['subject'],'message'=>$message,'image_url'=>$campaign['image_url']??null,'image_alt'=>$campaign['image_alt']??null,'card_link_url'=>!empty($campaign['image_url'])?(!empty($campaign['card_link_url'])?$referralUrl.'&dest=card':$referralUrl):null,'referral_url'=>$referralUrl,'referrer_name'=>$referrer['name'],'unsubscribe_url'=>$base.'/email/descadastrar/'.$lead['unsubscribe_token']]);$update->execute(['p'=>Encryption::encrypt($p),'id'=>$job['id']]);$updated+=$update->rowCount();}
        return $updated;
    }

    private function countQueuedCampaignJobs(\PDO $pdo,int $campaignId):int
    {
        $jobs=$pdo->query("SELECT payload_encrypted FROM jobs WHERE tenant_id IS NULL AND type='marketing.lead_email' AND status='queued' ORDER BY id")->fetchAll();$count=0;
        foreach($jobs as $job){try{$p=Encryption::decrypt($job['payload_encrypted']);}catch(\Throwable){continue;}if((int)($p['campaign_id']??0)===$campaignId)$count++;}
        return $count;
    }
    ...
    {
        if(!preg_match('/^[a-f0-9]{16}$/',$code))HttpException::abort(404,'Indicação inválida.');
        $pdo=Database::connection();
        $q=$pdo->prepare("SELECT rp.user_id FROM marketing_referral_profiles rp JOIN users u ON u.id=rp.user_id JOIN user_roles ur ON ur.user_id=u.id JOIN roles r ON r.id=ur.role_id WHERE rp.referral_code=:code AND rp.active=1 AND u.status='active' AND u.tenant_id IS NULL AND r.slug IN('master','support','commercial') LIMIT 1");
        $q->execute(['code'=>$code]);$referrerId=(int)$q->fetchColumn();if(!$referrerId)HttpException::abort(404,'Indicação inválida.');

        $campaignId=filter_var($_GET['c']??null,FILTER_VALIDATE_INT)?:null;
        $leadId=filter_var($_GET['l']??null,FILTER_VALIDATE_INT)?:null;
        if($campaignId){$valid=$pdo->prepare('SELECT 1 FROM marketing_campaign_referrers WHERE campaign_id=:c AND referrer_user_id=:u');$valid->execute(['c'=>$campaignId,'u'=>$referrerId]);if(!$valid->fetchColumn())$campaignId=null;}
        if($leadId){$valid=$pdo->prepare('SELECT 1 FROM marketing_leads WHERE id=:id');$valid->execute(['id'=>$leadId]);if(!$valid->fetchColumn())$leadId=null;}

        $token=bin2hex(random_bytes(32));$ip=(string)($_SERVER['REMOTE_ADDR']??'');
        $pdo->prepare("INSERT INTO marketing_referral_visits(referrer_user_id,campaign_id,lead_id,visit_token_hash,ip_hash,user_agent,clicked_at)VALUES(:u,:c,:l,:token,:ip,:ua,NOW())")
            ->execute(['u'=>$referrerId,'c'=>$campaignId,'l'=>$leadId,'token'=>hash('sha256',$token),'ip'=>$ip!==''?hash('sha256',$ip):null,'ua'=>mb_substr((string)($_SERVER['HTTP_USER_AGENT']??''),0,500)]);
        setcookie('ap_ref',$token,['expires'=>time()+2592000,'path'=>'/','secure'=>!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off','httponly'=>true,'samesite'=>'Lax']);

        $destination='/cadastro?indicacao='.$code;
        if(($campaignId??0)>0&&($_GET['dest']??'')==='card'){
            $q=$pdo->prepare("SELECT card_link_url FROM marketing_campaigns WHERE id=:id AND deleted_at IS NULL LIMIT 1");
            $q->execute(['id'=>$campaignId]);$url=trim((string)$q->fetchColumn());
            if($url!==''&&filter_var($url,FILTER_VALIDATE_URL)){
                $parts=parse_url($url);
                if(in_array(strtolower((string)($parts['scheme']??'')),['http','https'],true))$destination=$url;
            }
        }
