...
final class BarberRecurringReconciliationService
{
    ...
    {
        $pdo=Database::connection();$params=[];$sql="SELECT rs.*,pc.credentials_encrypted,pc.status connection_status,cm.status membership_status FROM tenant_recurring_subscriptions rs JOIN tenants t ON t.id=rs.tenant_id AND COALESCE(t.is_demo,0)=0 JOIN tenant_payment_connections pc ON pc.id=rs.connection_id AND pc.tenant_id=rs.tenant_id JOIN customer_memberships cm ON cm.id=rs.reference_id AND cm.tenant_id=rs.tenant_id WHERE rs.reference_type='customer_membership' AND rs.status IN('pending','authorized') AND cm.status='active' AND cm.billing_mode='provider' AND pc.provider='mercadopago' AND pc.status='connected'";if($tenantId){$sql.=' AND rs.tenant_id=:tenant';$params['tenant']=$tenantId;}$sql.=' ORDER BY rs.id';$q=$pdo->prepare($sql);$q->execute($params);$rows=$q->fetchAll();$checked=0;$applied=0;$errors=0;
