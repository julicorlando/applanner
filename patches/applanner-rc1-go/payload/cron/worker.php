...
while($processed<25){
    ...
    $job=$pdo->query("SELECT * FROM jobs WHERE status='queued' AND available_at<=NOW() ORDER BY id LIMIT 1 FOR UPDATE SKIP LOCKED")->fetch();
...
function sendLeadMarketing(PDO $pdo,array $payload):void{
    if(!filter_var($payload['to']??'',FILTER_VALIDATE_EMAIL)||empty($payload['subject'])||empty($payload['message'])||empty($payload['unsubscribe_url']))throw new RuntimeException('Campanha de marketing inválida.');$smtp=platformSecret($pdo,'platform.lead_marketing_smtp');$referral=!empty($payload['referral_url'])?"\n\nConheça o ApPlanner e comece seu teste:\n".$payload['referral_url']."\n\nIndicação: ".($payload['referrer_name']??'Equipe ApPlanner'):'';$body=(string)$payload['message'].$referral."\n\n---\nVocê recebeu esta mensagem por estar na lista de contatos do ApPlanner.\nNão deseja mais receber novidades? Acesse: ".$payload['unsubscribe_url'];(new App\Services\SmtpProvider())->send($smtp,$payload['to'],(string)$payload['subject'],$body);
}
