...
while($processed<25){
    ...
    try{
        ...
        if($job['type']==='marketing.lead_email'){
            $marketingSent=sendLeadMarketing($pdo,$payload);
            if($marketingSent)$pdo->prepare("UPDATE marketing_deliveries SET status='sent',sent_at=NOW(),error_message=NULL,updated_at=NOW() WHERE id=:id")->execute(['id'=>(int)$payload['delivery_id']]);
            else $pdo->prepare("UPDATE marketing_deliveries SET status='skipped',error_message='Campanha inativa ou removida.',updated_at=NOW() WHERE id=:id AND status='queued'")->execute(['id'=>(int)$payload['delivery_id']]);
        ...
        }elseif($job['type']==='meta.capi'){
            (new App\Services\MetaConversionService())->send($payload);
        }else{
...
function sendLeadMarketing(PDO $pdo,array $payload):bool{
    ...
    $campaignId=(int)($payload['campaign_id']??0);if($campaignId>0){$q=$pdo->prepare("SELECT active,deleted_at FROM marketing_campaigns WHERE id=:id LIMIT 1");$q->execute(['id'=>$campaignId]);$campaign=$q->fetch();if(!$campaign||(int)$campaign['active']!==1||!empty($campaign['deleted_at']))return false;}
    $smtp=platformSecret($pdo,'platform.lead_marketing_smtp');$render=(new App\Services\EmailMarketingTemplateService())->build($payload);(new App\Services\SmtpProvider())->sendHtml($smtp,$payload['to'],(string)$payload['subject'],$render['text'],$render['html']);return true;
}
