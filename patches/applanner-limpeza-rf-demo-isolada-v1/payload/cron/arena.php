...
if(!CronLock::acquire('arena')){echo "Arena: já em execução.\n";exit(0);}
...
$tenants = $pdo->query(
    "SELECT DISTINCT t.id FROM tenants t
     ...
     WHERE t.status IN('trial','active') AND t.deleted_at IS NULL AND COALESCE(t.is_demo,0)=0 AND COALESCE(tm.enabled,pm.enabled,0)=1"
)->fetchAll(PDO::FETCH_COLUMN);
$service = new ArenaMaintenanceService();
$summary = ['tenants'=>0,'expired'=>0,'membership_generated'=>0,'membership_conflicts'=>0,'waitlist_offers'=>0,'reminders'=>0,'class_billing'=>0,'crm_returns'=>0,'idle_alerts'=>0];
foreach ($tenants as $tenantId) {
    try {
        $result = $service->runTenant((int)$tenantId); $summary['tenants']++;
        foreach (['expired','membership_generated','membership_conflicts','waitlist_offers','reminders','class_billing','crm_returns','idle_alerts'] as $key) $summary[$key] += (int)($result[$key] ?? 0);
    } catch (Throwable $e) {
