...
final class LeadMarketingController
{
    ...
    public function index():void
    {
        ...
        $platformUsers=$pdo->query("SELECT DISTINCT u.id,u.name,u.email,r.slug role_slug FROM users u JOIN user_roles ur ON ur.user_id=u.id JOIN roles r ON r.id=ur.role_id WHERE u.tenant_id IS NULL AND u.status='active' AND r.slug IN('master','support','commercial') ORDER BY u.name")->fetchAll();$profileInsert=$pdo->prepare("INSERT IGNORE INTO marketing_referral_profiles(user_id,referral_code,active,created_at,updated_at)VALUES(:u,:code,1,NOW(),NOW())");foreach($platformUsers as $staff)$profileInsert->execute(['u'=>$staff['id'],'code'=>bin2hex(random_bytes(8))]);
        ...
        $leads=$pdo->query("SELECT * FROM marketing_leads ORDER BY id DESC LIMIT 300")->fetchAll();$campaigns=$pdo->query("SELECT c.*,u.name author_name,ru.name referrer_name FROM marketing_campaigns c JOIN users u ON u.id=c.created_by LEFT JOIN marketing_campaign_referrers cr ON cr.campaign_id=c.id LEFT JOIN users ru ON ru.id=cr.referrer_user_id ORDER BY c.id DESC LIMIT 80")->fetchAll();
        View::render('master/lead-marketing',['title'=>'E-mail marketing','smtp'=>$smtp,'smtpConfigured'=>!empty($smtp['host'])&&!empty($smtp['from_email']),'stats'=>$stats,'leads'=>$leads,'campaigns'=>$campaigns,'staff'=>$staff]);
    }
