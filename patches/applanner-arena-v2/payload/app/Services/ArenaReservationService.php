...
final class ArenaReservationService
{
    public function create(
        ...
        try {
            ...
            if (!$availability->free($tenantId, $courtId, $start, $end, (int)$courtRow['interval_minutes'], null, $excludeWaitlistId)) {
            ...
            $insert = $pdo->prepare(
                "INSERT INTO sports_reservations
                 (public_id,tenant_id,court_id,modality_id,customer_id,customer_name,customer_phone,customer_email,starts_at,ends_at,duration_minutes,price_per_hour,total_amount,base_total_amount,pricing_multiplier,pricing_details_json,deposit_amount,status,payment_method,payment_status,manage_token_hash,recurrence_group,source,notes,terms_accepted_at,confirmed_at,created_by,created_at,updated_at)
                 VALUES(:public,:tenant,:court,:modality,:customer,:name,:phone,:email,:starts,:ends,:duration,:hour_price,:total,:base_total,:pricing_multiplier,:pricing_details,:deposit,:status,:method,:payment,:token,:group_id,:source,:notes,NOW(),:confirmed,:user_id,NOW(),NOW())"
            );
            $insert->execute([
                ...
                'total' => $total,
                'base_total' => $quote ? (float)($quote['base_total'] ?? $total) : $total,
                'pricing_multiplier' => $quote ? (float)($quote['multiplier'] ?? 1) : 1,
                'pricing_details' => $quote ? json_encode($quote['adjustments'] ?? [], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR) : null,
                'deposit' => $depositAmount,
                'status' => $status,
                'method' => in_array($paymentMethod, ['pix','card'], true) ? $paymentMethod : 'onsite',
            ...
            if ($quote && abs((float)($quote['total'] ?? $total) - (float)($quote['base_total'] ?? $total)) > 0.009) {
                $pdo->prepare("INSERT INTO sports_dynamic_pricing_audit(tenant_id,reservation_id,base_total,final_total,multiplier,details_json,created_at) VALUES(:tenant,:reservation,:base,:final,:multiplier,:details,NOW())")
                    ->execute(['tenant'=>$tenantId,'reservation'=>$reservationId,'base'=>(float)($quote['base_total']??$total),'final'=>$total,'multiplier'=>(float)($quote['multiplier']??1),'details'=>json_encode($quote['adjustments']??[],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR)]);
            }
            $pdo->prepare(
