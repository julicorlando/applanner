...
final class PublicArenaExtrasController
{
    ...
    public function waitlist(string $slug): void
    {
        ...
        $pdo->prepare(
            "INSERT INTO sports_waitlist(public_token,tenant_id,customer_id,court_id,modality_id,customer_name,customer_phone,customer_email,preferred_date,preferred_start,preferred_end,flexibility_minutes,duration_minutes,status,notify_email,notify_whatsapp,notes,created_at,updated_at)
             VALUES(:token,:tenant,:customer,:court,:modality,:name,:phone,:email,:date,:start,:end,:flex,:duration,'waiting',:notify_email,:notify_whatsapp,:notes,NOW(),NOW())"
        )->execute([
            ...
            'notify_email' => isset($_POST['notify_email']) && $email!=='' ? 1 : 0, 'notify_whatsapp' => isset($_POST['notify_whatsapp']) ? 1 : 0,
