...
final class TenantPaymentWebhookController
{
    ...
    private function applyPaid(\PDO $pdo, array $tx): void
    {
        ...
        if ($tx['reference_type'] === 'reservation') {
            ...
            $this->financeIncome($pdo, $tenant, 'arena_reservation', $reference, 'Sinal de reserva Arena #' . $reference, $amount, 'pix');
            ...
            $this->financeIncome($pdo, $tenant, 'arena_game_player', $reference, 'Participação em racha #' . (int)$player['game_id'], $amount, 'pix');
        }
    }
